...
@[simp] def fml_force {states : Type} (k : S4 states) : states → fml → Prop
| s (var n)    := k.val n s
...
@[simp] def trans_size_left {st} (k : S4 st) : (Σ' (s) (φ : fml), fml_force k s φ) → ℕ := 
λ h, fml_size h.snd.fst

@[simp] def trans_size_right {st} (k : S4 st) : (Σ' (s : st) (φ : fml), force k s (fml.to_nnf φ)) → ℕ := 
λ h, fml_size h.snd.fst

theorem trans_left {st} (k : S4 st) : Π s φ, 
...
theorem trans_right {st} (k : S4 st) : Π s φ, 
...
theorem trans_iff {st} (k : S4 st) (s φ) : fml_force k s φ ↔ force k s (fml.to_nnf φ) := 
⟨trans_left k s φ, trans_right k s φ⟩

def fml_sat {st} (k : S4 st) (s) (Γ : list fml) : Prop := 
∀ φ ∈ Γ, fml_force k s φ
...
∀ (st) (k : S4 st) s, ¬ fml_sat k s Γ

theorem trans_sat_iff {st} (k : S4 st) (s) : Π Γ,
